...
public class JwtTokenProvider {
    ...
    public static final String HEADER_PREFIX = "Bearer"; // Bearer 토큰을 사용하기 위한 선언 값
    ...
    public String createToken(String userId, String roles) {
        ...
        return Jwts.builder()
                ...
                .signWith(secret, SignatureAlgorithm.HS256) // 사용할 암호화 알고리즘화
                .compact();
    }
    ...
    /**
     * JWT 토큰(Access Token, Refresh Token)의 userId, roles 가져옴
    ...
    public String resolveToken(ServerHttpRequest request, JwtTokenType tokenType) {
        ...
        if (tokenType == JwtTokenType.ACCESS_TOKEN) { // ACCESS일떄
            tokenName = accessTokenName;
        } else if (tokenType == JwtTokenType.REFRESH_TOKEN) { // REFRESH일떄
    ...
    public JwtStatus validateToken(String token) {
        ...
        if (token.length() > 0) {
            try {
                ...
                return JwtStatus.EXPIRED; // 혹시 몰라서 Exception으로 한번 더 체크
